"""
Kibana user enumeration

This module enumerates what users are available on an Kibana instance
...
class Users:
    ...
    """
    def __init__(self, args: object, ptjsonlib: object, helpers: object, http_client: object, base_response: object, print_label=True) -> None:
        self.args = args
    ...
    def run(self, users_r=None, roles_r=None) -> None:
        """
        Executes the Kibana user enumeration
        ...
        try:
            users_r = self.http_client.send_request(self.args.url + "api/console/proxy?path=/_security/user&method=GET",
                                                    method="POST",
                                                    headers=headers, follow_redirects=False)
        except requests.exceptions.RequestException as error_msg:
            ptprint(f"Error retrieving user list.", "ERROR",
                    not self.args.json, indent=4, colortext=True)

        if users_r.status_code != HTTPStatus.OK:
            ptprint(f"Could not enumerate users.",
                    "OK", not self.args.json, indent=4)
            ptprint(f"Received status code: {users_r.status_code}", "ADDITIONS", self.args.verbose, indent=4, colortext=True)
            return

        users = users_r.json()

        try:
            roles_r = self.http_client.send_request(self.args.url + "api/console/proxy?path=/_security/role&method=GET", method="POST",
                                                    headers=headers, follow_redirects=False)
        except requests.exceptions.RequestException as error_msg:
            ptprint(f"Error retrieving roles.", "ERROR",
                    not self.args.json, indent=4)


        if roles_r.status_code == HTTPStatus.OK:
            check_roles = True
        ...
        for entry in users:
            ...
            self._print_user(user_properties, check_roles, roles_r.json())


def run(args, ptjsonlib, helpers, http_client, base_response, print_label=True):
    """Entry point for running the user enumeration test"""
    Users(args, ptjsonlib, helpers, http_client, base_response, print_label).run()
